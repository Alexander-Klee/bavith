...
class VideoDecoder {
    ...
    public:
        ...
        std::expected<std::vector<uint8_t>, std::string> next_frame() const {
            ...
            if (av_read_frame(format_context, packet) >= 0) {
                ...
                if (packet->stream_index == video_stream_index) {
                    ...
                    if (ret < 0) {
                        return std::unexpected("failed to recieve frame");
                        // switch (ret) {
                        //     case AVERROR(EAGAIN):
                        //         std::cerr << "AVERROR(EAGAIN)" << std::endl;
                        //         break;
                        //     case AVERROR_EOF:
                    ...
                    std::cout << "width: " << get_width() << " height: " << get_height() << " frame num: " << decoder_context->frame_num << std::endl;
                    ...
                    ret = av_image_copy_to_buffer(buf.data(), buf_size,
                        ...
                        pixel_format, get_width(), get_height(), 1);
